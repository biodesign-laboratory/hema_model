...
function biomodel(dx, x, p, t)
    Hq=x[1];Hm=x[2];N=x[3];P=x[4];A=x[5];SCSF=x[6];K=x[7];Q=x[8];S=x[9];U=x[10]
    ...
    dx[1]=dHq;dx[2]=dHm;dx[3]=dN;dx[4]=dP;dx[5]=dA;dx[6]=dSCSF;dx[7]=dK;dx[8]=dQ;dx[9]=dS;dx[10]=dU
    return dx
end
