...
def simulate_TEST(init_values, rates, timestep_size, TFinal, path_repeat_t, path_repeat_size):
    '''
    ...
    # below are the arrays for diagnostic purposes
    #dA_output = [0]
    dE_star_output = [0]
    ...
    for x in timesteps[1:]:
        ...
        "Function for S_AS"     # this needs more work, reverted back to constant for now
        ...
        "Function for S_PS -- WIP"
        
        #---------- 2b. Calculating derivatives ---------------------
        
        dHdt = E*(R + H_stable) - D                                     # HSPC derivative
        
        dNdt = g_N*N - ((k_nq * Q) + (k_ns * S)) * (1-(N/N_inf))        # Pathogen derivative
        ...
        #dA_output.append(dAdt)
        dE_star_output.append(dE_star)
    ...
    output = np.array([H_output, N_output, P_output, A_output, S_output, Q_output, U_output, T_output, I_output, dE_star_output])
