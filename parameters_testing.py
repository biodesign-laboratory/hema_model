...
#         10000 * np.random.rand(runs),  # Quiescent HSPCs
#         5000 * np.random.rand(runs),      # Proliferating HSPCs
#         0 * np.random.rand(runs),      # PAMPs (Pathogens)
#         10000 * np.random.rand(runs),      # Pro-inflammatory Cytokines
#         10000 * np.random.rand(runs),      # Anti-inflammatory Cytokines
#         10000 * np.random.rand(runs),  # Stem Cell Supporting Factors
#         0 * np.random.rand(runs),      # DAMPs (Tissue Damage)
#         500 * np.random.rand(runs),      # Activated leukocytes
#         500 * np.random.rand(runs),   # Stable leukocytes
#         500 * np.random.rand(runs),       # Suppressor leukocytes
#         1 * np.random.rand(runs),      # MDSC
#         5000 * np.random.rand(runs),  # MF
...
for key in key_names_3:
    ...
    parameter_range = np.linspace(gv.parameter_range_global[key][0], gv.parameter_range_global[key][1], experiments)
    parameters_3_local = gv.averaged_parameters.copy()
    start_key = time.time()
    for h in range(experiments):
        parameters_3_local[key] = parameter_range[h]
        gv.parameters_3_global = parameters_3_local
        for i in range(runs):
            sol = solve_ivp(PL.model_3_derivatives, [0, t_final], init_state_3[:, i])
            print(f"Run {i} output successfully computed")
            sol_total[h][i] = sol
        # parameters_3_local[key] = gv.averaged_parameters[key]
...
for key in key_names_3:
    parameter_range = np.linspace(gv.parameters_3_global[key] / 8, (gv.parameters_3_global[key] * 15) / 8, experiments)
    gv.parameters_3_global = gv.parameters_3_global_bk.copy()
    start_key = time.time()
    for h in range(experiments):
...
for i in range(3):
    sol = solve_ivp(PL.model_3_derivatives, [0, t_final], init_state_3[i], dense_output=True, method='LSODA')
    sol_total = sol
