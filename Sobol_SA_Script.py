...
# this script is specifically meant to be used with MDSC model using the Sobol sensitivity method
...
delta_t = 1             # timestep size for sobol index calculation, NOT the timestep used in simulation runs which is fixed to 0.01
...
if run_sensitivity_analysis:    # run sensitivity analysis and save relevant sensitivity index to .csv files with delimiter='\t\
    ...
    for t in range(0, int(nTimesteps - init_time), 1):     # calculate sobol indices from init_time -> end of simulation with 1 timestep in between each
        ...
        SI_HQ = sobol_analyze.analyze(problem_IHD, IHD_out[:, 0, t], calc_second_order=True)    # int((init_time + t)/0.01)]
        ...
        total_Si_MF, first_Si_MF, second_Si_MF = SI_MF.to_df()
        total_Si_I, first_Si_I, second_Si_I = SI_I.to_df()

        total_SI_HQ.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HQ_out', 'total_order', f'total_SI_HQ_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_SI_HQ.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HQ_out', 'first_order', f'first_SI_HQ_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_SI_HQ.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HQ_out', 'second_order', f'second_SI_HQ_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        
        total_SI_HM.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HM_out', 'total_order', f'total_SI_HM_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_SI_HM.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HM_out', 'first_order', f'first_SI_HM_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_SI_HM.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'HM_out', 'second_order', f'second_SI_HM_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        ...
        total_Si_A.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'A_out', 'total_order', f'total_Si_A_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_A.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'A_out', 'first_order', f'first_Si_A_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_A.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'A_out', 'second_order', f'second_Si_A_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        '''total_Si_SCSF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'SCSF_out', 'total_order', f'total_Si_SCSF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_SCSF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'SCSF_out', 'first_order', f'first_Si_SCSF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_SCSF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'SCSF_out', 'second_order', f'second_Si_SCSF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')'''

        total_Si_K.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'K_out', 'total_order', f'total_Si_K_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_K.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'K_out', 'first_order', f'first_Si_K_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_K.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'K_out', 'second_order', f'second_Si_K_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        total_Si_S.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'S_out', 'total_order', f'total_Si_S_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_S.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'S_out', 'first_order', f'first_Si_S_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_S.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'S_out', 'second_order', f'second_Si_S_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        total_Si_Q.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'Q_out', 'total_order', f'total_Si_Q_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_Q.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'Q_out', 'first_order', f'first_Si_Q_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_Q.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'Q_out', 'second_order', f'second_Si_Q_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        total_Si_U.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'U_out', 'total_order', f'total_Si_U_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_U.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'U_out', 'first_order', f'first_Si_U_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_U.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'U_out', 'second_order', f'second_Si_U_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        total_Si_MDSC.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MDSC_out', 'total_order', f'total_Si_MDSC_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_MDSC.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MDSC_out', 'first_order', f'first_Si_MDSC_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_MDSC.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MDSC_out', 'second_order', f'second_Si_MDSC_{exp_num}_{int(t + init_time)}.csv'), sep='\t')

        total_Si_MF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MF_out', 'total_order', f'total_Si_MF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        first_Si_MF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MF_out', 'first_order', f'first_Si_MF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
        second_Si_MF.to_csv(os.path.join(script_dir, f'Experiment_{exp_num}', 'MF_out', 'second_order', f'second_Si_MF_{exp_num}_{int(t + init_time)}.csv'), sep='\t')
