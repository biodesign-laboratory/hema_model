...
output_to_csv = False
save_init_states = False                # save initial conditions to .csv
save_parameters = False                # save parameters to .csv
save_hyperparams = False                # save hyperparameters to .csv
...
read_from_hyper_loc = Path.cwd() / 'presets' / f'hyper_preset_{run_number}.csv'         # .csv containing hyperparameters to read from, put the path here
read_from_param_loc = Path.cwd() / 'presets' / f'parameter_preset_{run_number}.csv'        # .csv containing model paramaters to read from, put the path here
read_from_init_loc = Path.cwd() / 'presets' / f'init_val_preset_{run_number}.csv'         # .csv containing model initial values to read from, put the path here
...
if not read_from_hyper:             # set hyperparameters manually
    ...
    if nosocomial_start <= t_infection_start:
        raise Exception("Nosocomial infection must occur after initial infection.")
...
if not read_from_init:        # set initial values manually
    ...
    if model == 2:
        ...
        output_keys = ['HST', 'MPP', 'N', 'P', 'A', 'SCSF', 'K', 'Q', 'S', 'U']
        ...
        output_keys = ['HST', 'MPP', 'N', 'P', 'A', 'SCSF', 'K', 'Q', 'S', 'U', 'MDSC', 'EN']
...
# ------- 1. Running ODE solver (can easily swap this out for something like scpipy's solve_ivp() for faster solving) -------
...
for i in range(runs):
    ...
    if model == 2:
        ...
        outputs[i, :, :] = data[0]
        print(f"Run {i+1} output successfully computed; time elapsed: {end - start}s")
...
if output_to_csv:
    ...
    for i in range(runs):
        ...
        if model == 2:
            ...
            df = pd.DataFrame(outputs[i].T, columns=['HST', 'MPP', 'N', 'P', 'A', 'SCSF', 'K', 'Q', 'S', 'U', 'MDSC', 'EN', 'I'], index=list(map(str, timesteps)))
            df.to_csv(path / f'SIM_{run_number}_{i}_output.csv')
...
if plot_outputs:
    ...
    axs1[2].set_ylim((0, 60_000))        # optional; set y-limit for pathogen graph (useful if N(t) grows to carrying capacity)
    axs3[0].set_ylim((0, 150_000))        # optional; set y-limit for tissue damage graph (useful if K(t) grows large)
    axs5.plot(timesteps, np.zeros(len(timesteps)) + parameters['I_crit'], color=(0, 0, 0, 0.5), linestyle='--')
...
# output chronic infection 
if plot_chronic:

    for i in range(runs):
        
        chronic_test_arr = np.array(PL.chronic_infection_flagger(100, 200_000, 3000, 0.01, outputs[i, 2], derivatives[i, 2]))
        zeros_mask = chronic_test_arr == 0
        ones_mask = chronic_test_arr == 1
        t_zeros = timesteps[zeros_mask]
        t_ones = timesteps[ones_mask]

        plt.scatter(t_zeros, outputs[i,2][zeros_mask], color='r', s=0.1)
        plt.scatter(t_ones, outputs[i,2][ones_mask], color='g', s=0.1)

    plt.ylim(0, 150_000)
    plt.show()

# output pathogen derivative (troubleshoot chronic flagging tool)
for i in range(runs):
    
    plt.plot(timesteps, derivatives[i, 2])

plt.plot(timesteps, np.zeros(len(timesteps)) + 2000, linestyle='--')
plt.show()
