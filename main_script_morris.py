...
if generate_mu_vs_sigma:
    ...
    for i, output in enumerate(output_names):
        ...
        # this code here will sort and split the data into two to increase readability of graphs
        ...
        for n, df in enumerate([df_sorted_lowest, df_sorted_mid, df_sorted_highest]):        # creates 2 separate figures
            ...
            plt.title(fr"$\mu^*$ vs $\sigma$ (${output_names_laTex[i]}({t}$))")
            ...
            plt.savefig(os.path.join(path, f'{output}_{exp_num}_{t}_{n}.png'))
            plt.close()
            #plt.show()